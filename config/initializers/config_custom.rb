#If the RAILS_CONFIGSET environment variable is set and equal to 'set_name', then do the following:
...
#   credentials into each object of an array of objects.
...
if configset = ENV['RUBY_CONFIGSET']
  config_file = File.join(Rails.root, 'config', 'settings', 'configsets', "#{configset}.yml")
  if File.exist?(config_file)
    Settings.add_source!(config_file)
  end
  credential_configset = (Rails.application.credentials.configsets[configset] rescue nil)
